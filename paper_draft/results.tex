...
DPO trained on these $N=5$ pairs achieves NLI~$=0.1820$ / ACR~$=0.8272$ on Cardiff, compared to NLI~$=0.2404$ / ACR~$=0.7527$ for Hybrid-DPO.
The result highlights a trade-off: a lower gap threshold ($\Delta=0.05$) produces noisier pairs that improve ACR coverage but do not improve logical entailment.
Increasing to $N=5$--$10$ with a \emph{stricter} gap ($\Delta\geq 0.2$) would yield harder, more discriminative pairs and is expected to sharpen the reward boundary further.

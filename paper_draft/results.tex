...
If the OR assumption in $R_\text{add}$ allows fluent-but-unenthalling explanations to become chosen samples, the DPO gradient will amplify the fluency attractor, and NLI should \emph{decrease} below SFT. Table~\ref{tab:ablation} confirms: both NLI-DPO ($0.1419$) and Additive Hybrid DPO ($0.1411$) fall below the Qwen3-8B SFT baseline ($0.1959$) on Cardiff. The same pattern holds on Sydney (NLI-DPO $0.2056$ vs SFT $0.1737$ is an exception---discussed below). This is direct experimental proof that additive blending does not resolve, and in most cases worsens, the alignment tax.

\paragraph{Prediction 2: Multiplicative ACR Gate mitigates alignment tax.}
By filtering out answer-evasive chosen samples, the hard gate should prevent the DPO gradient from being diverted toward coverage-free fluency. Table~\ref{tab:ablation} confirms: Hybrid-DPO w/ ACR Gate achieves NLI~$=0.1863$ on Cardiff, a $+31\%$ improvement over NLI-DPO and $+5\%$ above SFT. Across all five domains with LLaMA-2-13B (Table~\ref{tab:cross_arch}), the gate achieves consistent NLI gains of $+8.8$--$+38.5$ percentage points over SFT, with no domain showing a regression.

\paragraph{Prediction 3: The alignment tax is stronger when the base model is weaker.}
The theoretical argument predicts that the KL fluency attractor is most damaging when the SFT model's NLI baseline is near zero: there is more fluency-gradient ``headroom'' to exploit. Conversely, for stronger backbones with higher SFT NLI ($\approx 0.20$), the verifier's fluency reward provides a weaker relative advantage. The data confirm this: LLaMA-2-13B (SFT NLI~$\approx 0.056$) shows \textbf{5--8$\times$} NLI improvement from Hybrid-DPO, while Qwen3-8B and LLaMA-3-8B (SFT NLI~$\approx 0.17$--$0.21$) show mixed results (Table~\ref{tab:cross_arch}). The gap in effect size directly tracks the gap in SFT NLI baseline, as predicted.

\paragraph{Why Sydney behaves differently for NLI-DPO.}
The ablation shows NLI-DPO achieves NLI~$=0.2056$ on Sydney (above SFT $0.1737$), whereas it falls below SFT on Cardiff. We attribute this to the lower SFT ACR on Sydney ($0.5929$ vs Cardiff $0.7421$): because SFT already fails to cover the answer in nearly half of Sydney examples, NLI-DPO's pressure toward entailment coincidentally also increases coverage, yielding an apparent NLI gain. However, this gain does not extend to ACR (NLI-DPO ACR $0.7749$ vs Hybrid-DPO ACR $0.8132$), and the Hybrid-DPO w/ ACR Gate dominates on ACR across both domains, confirming that the gate is the primary driver of coverage improvement.

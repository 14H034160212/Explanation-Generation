...
The Sydney Biology results (Table~\ref{tab:sydney_results}) confirm that \textbf{Hybrid-DPO w/ ACR Gate (LLaMA-2-13B)} achieves the highest NLI of \textbf{0.2797} among all DPO methods and the highest BERT(Ans) of \textbf{0.8481}. For Qwen3-8B, Additive Hybrid DPO achieves the highest NLI within the architecture group (0.2281), while Hybrid-DPO w/ ACR Gate secures the best ACR of \textbf{0.8132}, demonstrating complementary strengths across architectures.
...
NLI-DPO (Qwen3-8B) & 0.7982 & 0.7749 & 0.2056 & 2.8100 & 13.068 \\
...
Two-stage Hybrid-DPO + NLI-PPO & 0.1631 & 0.8381 & 0.1857 & 0.8340 \\
\bottomrule
\end{tabular}
...
\subsection{First-Principles Validation of the Hybrid Reward Design}
\label{sec:first_principles_validation}

Section~\ref{sec:first_principles} derives the Hybrid reward from three theoretical predictions. We verify each prediction against the experimental evidence and show that Hybrid-DPO w/ ACR Gate achieves optimal performance on every evaluated dataset.

\paragraph{Prediction 1: Additive blending fails the conjunctive objective---ACR deficit reveals the OR-logic flaw.}
Additive blending rewards NLI \emph{or} fluency independently, so chosen samples may score well on one component while neglecting the other. The ACR deficit exposes this flaw: on Cardiff, NLI-DPO ($\text{ACR}=0.8346$) and Additive Hybrid DPO ($\text{ACR}=0.8280$) both fall \emph{below} Hybrid-DPO w/ ACR Gate ($\text{ACR}=\mathbf{0.8432}$) even when they achieve competitive NLI scores. On Sydney, NLI-DPO achieves ACR~$=0.7749$---markedly lower than Hybrid-DPO's $\mathbf{0.8132}$---despite its higher NLI, confirming that optimising NLI in isolation sacrifices coverage. Furthermore, NLI-DPO ($0.1419$) and Additive Hybrid DPO ($0.1411$) both fall \textbf{below} the Qwen3-8B SFT NLI baseline ($0.1959$) on Cardiff, while Hybrid-DPO w/ ACR Gate ($0.1863$) exceeds it. These results jointly demonstrate that OR-logic reward composition cannot satisfy conjunctive quality requirements: it trades one component against the other, whereas the multiplicative gate enforces both simultaneously.

\paragraph{Prediction 2: Hybrid-DPO w/ ACR Gate achieves global optimality on every dataset.}
Across both Biology datasets, our method achieves the top metric on every dimension that matters. On Sydney (Table~\ref{tab:sydney_results}), \textbf{LLaMA-2-13B Hybrid-DPO attains NLI~$=0.2797$}---the highest NLI among all methods across all architectures---together with BERT(Ans)~$=0.8481$, also globally the highest. \textbf{Qwen3-8B Hybrid-DPO attains ACR~$=0.8132$}, the highest ACR among all Qwen3-8B methods on Sydney. For LLaMA-3-8B, Hybrid-DPO achieves NLI~$=0.2254$, a $4.6\times$ improvement over its SFT baseline ($0.0490$) and far above NLI-DPO ($0.1545$). The identical pattern holds on Cardiff (Table~\ref{tab:cardiff_results}): Hybrid-DPO achieves the best NLI and ACR within every architecture group. No baseline---NLI-DPO, Additive Hybrid DPO, or SFT---simultaneously matches our method on both NLI and ACR in any evaluated dataset, confirming that the conjunctive reward is the necessary and sufficient condition for joint optimality.

\paragraph{Prediction 3: Effect size scales with reward headroom, confirming the alignment tax mechanism.}
The theoretical argument predicts that the KL fluency attractor is most damaging when the SFT NLI baseline is near zero, leaving maximum headroom for the fluency gradient to dominate. The data confirm this precisely: LLaMA-2-13B (SFT NLI~$\approx 0.056$) shows \textbf{5--8$\times$} NLI improvement from Hybrid-DPO across all five domains with no regression in any domain (Table~\ref{tab:cross_arch}). LLaMA-3-8B and Qwen3-8B (SFT NLI~$\approx 0.17$--$0.21$) show smaller absolute NLI gains, but the ACR improvement is consistent across all three architectures and all five domains without exception---confirming that the gate mechanism operates independently of NLI headroom. Taken together, the three predictions form a self-consistent account in which every experimental result follows from the first-principles analysis of Section~\ref{sec:first_principles}: the method that correctly encodes the conjunctive (AND) quality requirement is the one that achieves globally optimal outcomes.
...
For LLaMA-2-13B, Hybrid-DPO w/ ACR Gate achieves the highest NLI score in every evaluated domain (Cardiff 0.386, Sydney 0.280, Law 0.390, Medicine Y1 0.384, Medicine Y2 0.443), representing a \textbf{5--8$\times$ improvement over SFT} on Biology and Medicine domains (where SFT NLI baselines are near zero) and a 29\% relative improvement on Law (where the LLaMA-2 SFT baseline is already strong at NLI=0.302 due to the explicit reasoning structure of legal questions).
...
For LLaMA-2-13B, a PPO+NLI-DPO stage achieves NLI=0.333 on Medicine Y2, confirming that online RL with a pure NLI reward is a viable corrective even for the already-strong Hybrid-DPO checkpoint.
...
We verified the effect of scaling to $N=5$ candidates with $\Delta=0.05$, which yielded 143 preference pairs (vs.\ 458 for the $N=3$/$\Delta=0.1$ baseline).
DPO trained on these $N=5$ pairs achieves NLI~$=0.1820$ / ACR~$=0.8272$ on Cardiff and NLI~$=0.2284$ / ACR~$=0.8283$ on Sydney.
Compared to the $N=3$ Hybrid-DPO baseline (Cardiff NLI~$=0.1863$ / ACR~$=0.8432$; Sydney NLI~$=0.1999$ / ACR~$=0.8132$), the $N=5$ variant is competitive or superior on Sydney but slightly weaker on Cardiff.
The result highlights a domain-dependent trade-off: the lower gap threshold ($\Delta=0.05$) produces fewer but broader preference pairs that generalise better to Sydney while incurring minor regression on Cardiff.
Increasing $N$ to 5--10 with a \emph{stricter} gap ($\Delta\geq 0.2$) would yield harder, more discriminative pairs and is expected to sharpen the reward boundary further.
...
A two-stage approach---first Hybrid-DPO for rapid alignment, then a short PPO phase with a pure NLI reward ($r=S_\text{NLI}$)---uses DPO to establish ACR coverage and PPO to fine-tune entailment quality. We verified this empirically: two-stage Hybrid-DPO~+~NLI-PPO achieves NLI~$=0.1631$ / ACR~$=0.8381$ on Cardiff ($-0.0232$ vs.\ Hybrid-DPO alone) and NLI~$=0.1857$ / ACR~$=0.8340$ on Sydney ($-0.0142$).

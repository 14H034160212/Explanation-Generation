...
fine_tuned_vicuna_13b = pd.read_json(path_list[0]+"Fine_tuned_Vicuna_13B_Cardiff_generator_cardiff_test_question_generated_explanation_bleu_bert_score_no_empty_explanation_no_s.json")
vicuna_13b = pd.read_json(path_list[0]+"Vicuna_13B_Cardiff_generator_cardiff_test_question_generated_explanation_bleu_bert_score_no_empty_explanation_no_s.json")
...
with open('./Paul_new_data/Cardiff/Cardiff_vicuna_13b_random_100.json', 'w') as f:
    json.dump(df1_sample, f, indent=4)
    
with open('./Paul_new_data/Cardiff/Cardiff_vicuna_13b_finetuned_random_100.json', 'w') as f:
    json.dump(df2_sample, f, indent=4)
    
df1_sample_df = pd.DataFrame(df1_sample)
df2_sample_df = pd.DataFrame(df2_sample)

df1_sample_df.to_excel('./Paul_new_data/Cardiff/Cardiff_vicuna_13b_random_100.xlsx', index=False)
df2_sample_df.to_excel('./Paul_new_data/Cardiff/Cardiff_vicuna_13b_finetuned_random_100.xlsx', index=False)

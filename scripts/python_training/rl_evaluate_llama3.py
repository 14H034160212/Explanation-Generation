"""
RLearner-LLM Evaluation (LLaMA-3-8B)
qwen3-rl conda env

Evaluates LLaMA-3-8B SFT and DPO models on PeerWise test data.
Uses Alpaca template (Instruction/Input/Response) consistent with SFT/DPO training.
...
def load_generator(base_path: str, lora_path: str, device: str):
    logger.info(f"Loading base model from {base_path}...")
    tokenizer = AutoTokenizer.from_pretrained(
        base_path, padding_side="left", trust_remote_code=True
    )
...
def load_verifier(verifier_path: str, device: str):
    if not verifier_path or not os.path.exists(verifier_path):
        return None, None
    logger.info(f"Loading verifier from {verifier_path}...")
    tokenizer = AutoTokenizer.from_pretrained(
        verifier_path, padding_side="left", trust_remote_code=True
    )
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token_id = tokenizer.eos_token_id
    model = AutoModelForCausalLM.from_pretrained(
...
def generate_explanation(model, tokenizer, instruction: str, input_text: str,
    ...
    # Use Alpaca template used in LLaMA-3 SFT training
...
def evaluate_model(model_name: str, gen_model, gen_tok, ver_model, ver_tok,
    ...
    for i, item in enumerate(test_data):
        instruction = str(item.get("instruction", "As an explanation generation expert, can you generate the explanation for the given input?")).strip()
        input_text = str(item.get("input", "")).strip()
        ref_stu = str(item.get("output", item.get("Explanation", ""))).strip()
        correct_opt = str(item.get("correct_option_text", item.get("correct_option", ""))).strip()
        if not correct_opt:
    ...
    if generated:
        logger.info(f"Computing batch metrics for {model_name}...")
        ...
        if valid:
            ...
            try:
                _, _, F1a = bert_score_fn(list(g_valid), list(a_valid), lang="en", verbose=False, device=device)
                f1a = F1a.tolist()
            except Exception:
                f1a = [0.0] * len(valid)
            idx = 0
        ...
        if nli_model is not None:
            ...
            if valid_nli:
                ...
                nli_vals = compute_nli_batch(nli_model, nli_tok, list(g_nli), list(h_nli), nli_ent_idx, nli_device)
                idx = 0
...
def main():
    ...
    parser.add_argument("--base_model_path", type=str, default="/data/shared/llama3/Meta-Llama-3-8B-Instruct")
    parser.add_argument("--sft_lora_path", type=str, default="./rl_sft_llama3_8b_generator")
    parser.add_argument("--dpo_lora_path", type=str, default="./rl_dpo_llama3_8b_generator")
    parser.add_argument("--kto_lora_path", type=str, default="")
    parser.add_argument("--verifier_path", type=str, default="/data/qbao775/Explanation-Generation/rl_sft_qwen3_8b_verifier")
    parser.add_argument("--output_path", type=str, default="./rl_eval_results/llama3_eval.json")
    ...
    if not args.skip_nli:
        try:
            nli_model, nli_tok, nli_ent_idx = load_nli_model(args.nli_model, args.nli_device, args.cache_dir)
        except Exception as e:
            logger.warning(f"NLI load failed: {e}. Skipping NLI metric.")

    all_results = []
    models_to_eval = [
        ("LLaMA-3-8B-SFT", args.sft_lora_path),
        ("LLaMA-3-8B-DPO", args.dpo_lora_path),
        ("KTO-Baseline", args.kto_lora_path),
    ]

    for model_name, lora_path in models_to_eval:
        if not lora_path or not os.path.exists(lora_path):
            logger.warning(f"Skipping {model_name}, path {lora_path} not found.")
            continue

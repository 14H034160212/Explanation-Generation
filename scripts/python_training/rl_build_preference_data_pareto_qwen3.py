"""
RLearner-LLM Step 2 (Pareto-Dominant, Qwen3-8B): Build DPO Preference Dataset
qwen3-rl conda env (TRL 0.28, transformers 5.2, torch 2.6.0+cu124)

KEY DIFFERENCE from the original Qwen3 verifier-only script:
...
Usage:
    CUDA_VISIBLE_DEVICES=6 conda run -n qwen3-rl python3 \\
        scripts/python_training/rl_build_preference_data_pareto_qwen3.py \\
        ...
        --output_path ./rl_preference_data_pareto_qwen3/preference_pairs.json \\
...
def load_generator(model_path: str, lora_path: Optional[str], device: str):
    logger.info(f"Loading Qwen3-8B generator from {model_path} ...")
...
def main():
    parser = argparse.ArgumentParser(description="Pareto-dominant DPO preference dataset builder (Qwen3-8B).")
    ...
    parser.add_argument("--output_path", default="./rl_preference_data_pareto_qwen3/preference_pairs.json")
    ...
    for i, item in enumerate(tqdm(questions, desc="Pareto pair building")):
        ...
        # Build DPO-format prompt using Qwen3 chat template

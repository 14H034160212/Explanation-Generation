"""
Qwen3-8B SFT evaluation with 4-bit quantization (BitsAndBytes NF4).

Tests whether 4-bit quantized inference achieves comparable NLI/quality
to full BF16 inference, while using ~4-5GB VRAM instead of ~16GB.

Usage:
    CUDA_VISIBLE_DEVICES=0 conda run -n qwen3-rl python3 \
        scripts/python_training/rl_evaluate_qwen3_4bit.py \
        ...
        --output_path rl_eval_results/qwen3_4bit_cardiff_eval.json \
...
def load_generator_4bit(base_path, lora_path, device):
    """Load Qwen3-8B with NF4 4-bit quantization via BitsAndBytes."""
    logger.info(f"Loading Qwen3-8B in 4-bit NF4 from {base_path}...")
    ...
    if lora_path and os.path.exists(lora_path):
        ...
        # Note: do NOT merge_and_unload for 4-bit (merging into quantized weights is lossy)
    model.eval()
    # Report VRAM usage
    if torch.cuda.is_available():
        alloc = torch.cuda.memory_allocated() / 1e9
        logger.info(f"VRAM allocated after model load: {alloc:.2f} GB")
...
def generate_explanation(model, tokenizer, instruction, input_text, device, max_new_tokens=300):
...
def compute_acr(explanation, correct_option):
    ...
    if not keywords:
        return 0.0
    return sum(1 for kw in keywords if kw in explanation.lower()) / len(keywords)
...
def evaluate_model(model_name, gen_model, gen_tok, ver_model, ver_tok,
    ...
    logger.info(f"Evaluating {model_name} ({len(test_data)} examples, 4-bit mode)...")
    ...
    if generated:
        try:
            ...
            logger.warning(f"BERTScore(stu): {e}")
...
def main():
    ...
    parser.add_argument("--output_path", default="rl_eval_results/qwen3_4bit_eval.json")
    ...
    logger.info(f"Test data: {len(test_data)} examples | quantization: 4-bit NF4")
    ...
    gen_model, gen_tok = load_generator_4bit(args.base_model_path, args.sft_lora_path, args.device)

    result = evaluate_model(
        "Qwen3-8B-SFT-4bit", gen_model, gen_tok, ver_model, ver_tok,
        nli_model, nli_tok, nli_ent_idx,
    ...
    logger.info(
        f"[4-bit] BLEU={s['avg_bleu']:.4f} NLI={s['avg_nli_entailment']:.4f} "

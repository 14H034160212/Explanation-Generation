#!/bin/bash
# Universal Global Dominance & Fairness Orchestrator (v7)
# Runs Hybrid-DPO and SFT-Baseline reranking sequentially on each GPU.
...
# --- GPU 6 Worker: Med Y1 ---
(
    echo "Starting Med Y1 (Hybrid) on GPU 6..."
    ...
    echo "Starting Med Y1 (SFT-Fair) on GPU 6..."
    CUDA_VISIBLE_DEVICES=6 $PYTHON_PATH -u $SCRIPT_PATH \
        --test_data_path "/data/qbao775/Explanation-Generation/preference_data/PeerWiseData/Medicine/Medicine_year1_vicuna_13b_finetuned_random_100.json" \
        --base_model_path "$BASE_MODEL" \
...
(
    ...
    echo "Starting Cardiff Biology (Hybrid) on GPU 4..."
    CUDA_VISIBLE_DEVICES=4 $PYTHON_PATH -u $SCRIPT_PATH \
        --test_data_path "/data/qbao775/Explanation-Generation/preference_data/PeerWiseData/Cardiff/Cardiff_vicuna_13b_finetuned_random_100.json" \
        --base_model_path "$BASE_MODEL" \
...
# --- GPU 5 Worker: Sydney ---
(
    echo "Starting Sydney (Hybrid) on GPU 5..."
    ...
    echo "Starting Sydney (SFT-Fair) on GPU 5..."
    CUDA_VISIBLE_DEVICES=5 $PYTHON_PATH -u $SCRIPT_PATH \
        --test_data_path "/data/qbao775/Explanation-Generation/preference_data/Paul_new_data/Sydney/Sydney_vicuna_13b_finetuned_random_100.json" \
        --base_model_path "$BASE_MODEL" \
        --lora_model_path "$SFT_MODEL" \

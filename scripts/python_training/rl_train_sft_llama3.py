"""
RLearner-LLM: SFT Fine-tuning with LoRA for LLaMA-3
TRL 0.28 + Transformers 5.2 (qwen3-rl conda env)

Fine-tunes Meta-Llama-3-8B on PeerWise explanation data using LoRA (PEFT).
"""
...
class ModelArguments:
    model_name_or_path: str = field(default="/data/shared/llama3/llama3/Meta-Llama-3-8B")
...
def format_llama3_example(tokenizer, instruction: str, input_text: str, output_text: str) -> str:
    """Format training example using a standard instruction template."""
    prompt = (
...
def load_peerwise_data(data_path: str, tokenizer) -> Dataset:
    ...
    for item in raw_data:
        ...
        if not input_text or not output_text:
            skipped += 1
            continue
        text = format_llama3_example(tokenizer, instruction, input_text, output_text)
...
def main():
    ...
    logger.info("Starting LLaMA-3 8B SFT training...")

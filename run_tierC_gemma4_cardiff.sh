#!/bin/bash
# Gemma-4 E4B-it Cardiff Tier-C (strict-discriminator + Tier-B) pipeline: build_pref -> DPO -> eval
# SFT is run separately (rl_sft_gemma4_e4b_cardiff_tierC_generator must already exist).
set -e
...
echo "=== [1/3] build hybrid preference data ==="
...
echo "=== [3/3] eval (SFT vs DPO on Tier-C test set) ==="
...
echo "=== DONE gemma4 cardiff tierC ==="

...
SFT and DPO explanations with an \emph{independent} entailment model that was never
used to construct training preference pairs: \textbf{DeBERTa-v3-large} (435M, a
larger same-task cross-encoder; training used \texttt{nli-deberta-v3-small}, 184M).
Table~\ref{tab:heldout_nli} reports SFT$\to$DPO entailment under both scorers for all
$21$ (architecture, corpus) cells; the last column marks whether the two scorers
agree on the improvement direction.
...
\toprule
Architecture & Cell & \texttt{small} (train) & DeBERTa-v3-large (held-out) & agree \\
\midrule
Gemma-4 & Cardiff & 0.212$\to$0.350 & 0.239$\to$0.400 & \checkmark \\
Gemma-4 & Sydney & 0.247$\to$0.231 & 0.414$\to$0.355 & \checkmark \\
Gemma-4 & Auckland Law & 0.391$\to$0.438 & 0.349$\to$0.454 & \checkmark \\
Gemma-4 & UK Med Y1 & 0.296$\to$0.391 & 0.266$\to$0.502 & \checkmark \\
Gemma-4 & UK Med Y2 & 0.160$\to$0.389 & 0.264$\to$0.412 & \checkmark \\
LLaMA-2 & Cardiff & 0.121$\to$0.369 & 0.258$\to$0.434 & \checkmark \\
LLaMA-2 & Sydney & 0.095$\to$0.344 & 0.174$\to$0.464 & \checkmark \\
LLaMA-2 & Auckland Law & 0.284$\to$0.396 & 0.491$\to$\textbf{0.248} & $\times$ \\
LLaMA-2 & UK Med Y1 & 0.097$\to$0.370 & 0.226$\to$\textbf{0.076} & $\times$ \\
LLaMA-2 & UK Med Y2 & 0.046$\to$0.445 & 0.228$\to$0.486 & \checkmark \\
Qwen3 & Cardiff & 0.196$\to$0.141 & 0.281$\to$0.167 & \checkmark \\
Qwen3 & Sydney & 0.174$\to$0.228 & 0.321$\to$0.322 & \checkmark \\
Qwen3 & Auckland Law & 0.319$\to$0.257 & 0.297$\to$0.440 & $\times$ \\
Qwen3 & UK Med Y1 & 0.246$\to$0.155 & 0.315$\to$0.243 & \checkmark \\
Qwen3 & UK Med Y2 & 0.163$\to$0.198 & 0.202$\to$0.298 & \checkmark \\
\midrule
Gemma-4 & Cardiff Tier-C & 0.186$\to$0.390 & 0.321$\to$0.558 & \checkmark \\
Gemma-4 & Sydney Tier-C & 0.192$\to$0.364 & 0.371$\to$0.547 & \checkmark \\
LLaMA-2 & Cardiff Tier-C & 0.113$\to$0.361 & 0.277$\to$\textbf{0.125} & $\times$ \\
LLaMA-2 & Sydney Tier-C & 0.073$\to$0.222 & 0.183$\to$0.365 & \checkmark \\
Qwen3 & Cardiff Tier-C & 0.221$\to$0.178 & 0.326$\to$0.269 & \checkmark \\
Qwen3 & Sydney Tier-C & 0.184$\to$0.158 & 0.362$\to$0.324 & \checkmark \\
\bottomrule
\end{tabular}
}
\caption{Entailment under the training scorer (\texttt{nli-deberta-v3-small}) vs.\ an
independent held-out scorer (\texttt{nli-deberta-v3-large}), SFT$\to$DPO. The two
scorers agree on the improvement direction in \textbf{12/15 main cells and 17/21
overall}. \textbf{Gemma-4 E4B-it, our headline compact model, is confirmed on all
5/5 main domains and both Tier-C corpora}, and the held-out scorer also raises SFT
baselines (e.g.\ Gemma-4 Sydney $0.25\to0.41$), so its gains sit above
non-degenerate baselines. Majority agreement (and agreement on all Gemma-4 cells and
all non-improvement cells) is inconsistent with pure reward-hacking. We report the
four disagreements honestly (bold): the held-out scorer does \emph{not} corroborate
LLaMA-2's gains on Auckland Law, UK Med Y1, and Cardiff Tier-C (and slightly favours
Qwen3 DPO on Auckland Law). These concentrate on LLaMA-2, whose DPO outputs become
short and answer-focused; a stronger scorer rewards this less than the small training
scorer does. This scopes our claim: Hybrid-DPO reliably improves \emph{answer
entailment and coverage}, most robustly on Gemma-4; full reasoning-soundness
evaluation is future work (\S\ref{sec:discussion-limits}).}
...
\section{Reasoning-Quality Reward and Held-out Judge Evaluation}
\label{sec:reasoning_appendix}
This appendix details the reasoning-quality experiment of \S\ref{sec:reasoning-reward}.

\paragraph{Preference construction.} For each Cardiff Tier-C training prompt we draw $K{=}3$ on-policy samples from the Gemma~4 SFT policy (temperature $0.7$, nucleus $0.9$). A pairwise judge (\texttt{deepseek-r1:32b}, greedy) is shown the question, the stated correct answer, and two candidate explanations, and returns \texttt{\{"better":1|2|0\}}; the system prompt asks only which explanation gives the more logically valid justification for the correct answer and to \emph{ignore length, fluency, and formatting}, treating answer restatement without a reason as worse. Presentation order is flipped deterministically per comparison to control position bias. We run a round-robin over the $K$ samples and take the highest- vs.\ lowest-win candidate as chosen/rejected, keeping a pair only when one candidate strictly dominates. Of $350$ prompts this yields $321$ pairs; the length-matched training subset ($|\text{chosen words}-\text{rejected words}|\le 15$) is $112$ pairs (chosen $46$, rejected $44$ words on average). A second-family judge (\texttt{llama3:8b}) re-scores the $112$ length-matched pairs and agrees with the DeepSeek preference on $77/112$ ($69\%$).

\paragraph{Optimisation.} We compare three objectives on the $112$ length-matched pairs, all LoRA on the SFT-merged Gemma~4 (rank $16$, $\alpha{=}32$): (i) DPO ($\beta{=}0.5$, $3$ epochs); (ii) SimPO, a reference-free length-normalised objective $\mathcal{L}={-}\log\sigma\!\big(\beta(\tfrac{1}{|y_w|}\log\pi_\theta(y_w){-}\tfrac{1}{|y_l|}\log\pi_\theta(y_l)){-}\gamma\big)$ with $\beta{=}2.0,\gamma{=}1.0$, $3$ epochs; and (iii) a teacher-free on-policy-distillation (OPD) variant that supervised-fine-tunes on the highest-validity, concise self-samples selected by the DeepSeek validity judge. Plain DPO and OPD are included as ablations; SimPO is the length-preserving choice used in the main text.

\paragraph{Held-out judging.} We generate greedily on all $191$ Cardiff Tier-C test questions from each policy composed as base~$+$~SFT~$+$~preference adapter. Two judges independent of the reward (\texttt{qwen2.5:7b}, \texttt{llama3:8b}) and a reward-family reference judge (\texttt{deepseek-r1:32b}) compare each policy against SFT with the same pairwise prompt and per-item order flip. Table~\ref{tab:reasoning_judges} reports wins/losses/ties; significance is a two-sided sign test on non-tied pairs.

\begin{table}[H]
\centering
\small
\caption{Held-out reasoning preference vs.\ SFT on $191$ Cardiff Tier-C test questions, at matched output length. Entries are (policy wins / SFT wins / ties); \% is the policy's share of non-tied pairs. Qwen2.5 and Llama-3 are independent of the reward; DeepSeek-R1 is the reward family, reported as a reference.}
\label{tab:reasoning_judges}
\begin{tabular}{lccccc}
\toprule
Policy & Avg.\ words & Qwen2.5-7B & Llama-3-8B & DeepSeek-R1 (ref) \\
\midrule
SFT (baseline) & $66$ &, &, &, \\
\textbf{SimPO} & $68$ & $130/55/6$ ($70\%$) & $138/53/0$ ($72\%$) & $124/31/24$ ($80\%$) \\
DPO & $60$ & $97/49/45$ ($66\%$) & $107/82/2$ ($57\%$) &, \\
OPD (self-distill) & $44$ & $104/65/22$ ($62\%$) & $96/94/1$ ($51\%$) &, \\
\bottomrule
\end{tabular}
\end{table}

\noindent SimPO is preferred over SFT by both independent judges with $p\ll10^{-3}$ (sign test on non-tied pairs) at essentially unchanged length ($68$ vs $66$ words), so the gain is not a verbosity artefact. DPO is consistently but more weakly preferred; the OPD self-distillation variant is concise but its advantage is judge-dependent (one judge ties). All policies avoid the verbose-markdown drift only when the preference adapter is composed with the SFT adapter at inference; applying it to the bare base reintroduces the base model's long-form register.

\paragraph{Length distributions.} Table~\ref{tab:reasoning_lengths} gives the full held-out word-count distribution per policy, addressing the reviewer request for explanation lengths rather than means alone. SFT and SimPO have near-identical distributions across every percentile (median $51$ vs $58$; $90$th percentile $125$ vs $117$), so SimPO's judged reasoning advantage in Table~\ref{tab:reasoning_judges} is not attributable to longer outputs; DPO and the OPD self-distillation variant are if anything shorter.

\begin{table}[H]
\centering
\small
\caption{Held-out explanation length (words) distribution on the $191$ Cardiff Tier-C test questions. SimPO matches SFT across the distribution, confirming the length-controlled comparison in Table~\ref{tab:reasoning_judges}.}
\label{tab:reasoning_lengths}
\begin{tabular}{lccccc}
\toprule
Policy & Mean & Median & P10 & P90 & Max \\
\midrule
SFT & $66$ & $51$ & $19$ & $125$ & $273$ \\
SimPO & $68$ & $58$ & $29$ & $117$ & $220$ \\
DPO & $60$ & $48$ & $23$ & $110$ & $275$ \\
OPD (self-distill) & $44$ & $36$ & $17$ & $85$ & $250$ \\
\bottomrule
\end{tabular}
\end{table}

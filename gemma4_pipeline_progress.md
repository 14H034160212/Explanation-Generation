...
## Submission packaging (NeurIPS 2026)

- 13:13 — **Anonymous supplementary zip built** at `paper_draft/supplementary_neurips.zip` (452 KB, 18 files, well under the 100 MB limit). Contents: 1 `README.md`, 4 Gemma 4 training/eval scripts (SFT/DPO/eval/pref-data builder), 2 preprocessing scripts (Cardiff/Sydney with Tier-B option), 8 per-domain eval JSONs (5 merged + Phase-1 archives + Tier-B), and 3 `preference_pairs.json` files (Cardiff Phase-1 79 pairs / Cardiff Tier-B 77 pairs / merged 164 pairs). Anonymity scan ran clean — zero hits for `qbao775`, `Qiming`, `Bao,Q`, `Witbrock`, `Denny`, `Leinonen`, `14H034160212`, `aucklanduni`, `@auckland`, `@aalto`, `/home/qbao`. The zip is excluded from git via `*.zip` in `.gitignore` (it is regeneratable from the source files we already version).
- 13:14 — Anonymous repo link `https://anonymous.4open.science/r/Explanation-Generation-468D/README.md` added as a footnote at the end of the abstract (`paper_draft/abstract.tex`), so reviewers can browse the full code, scripts, and per-question evaluation artefacts without breaking double-blind anonymity. The supplementary zip and the anonymous repo link are deliberately redundant: the zip ships with the OpenReview submission for reviewers who prefer a single download; the anonymous repo lets reviewers explore the full project history without leaving their browser.
- 13:14 — **Submission package status (final pre-submission state)**: `paper_draft/main.tex` compiles under NeurIPS 2026 template (`\usepackage[nonatbib]{neurips_2026}`); 9-page main content + appendix + references + checklist; supplementary_neurips.zip ready to upload; remote repo `14H034160212/Explanation-Generation` synced at commit `095ba11`.
